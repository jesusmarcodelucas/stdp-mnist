...
MNIST_data_path = r"c:\PYPROJECTS\myenv\stdp-mnist\MNIST_data\\"
...
#------------------------------------------------------------------------------
start = time.time()
training = get_labeled_data(MNIST_data_path)

end = time.time()
print(f'time needed to load training set: {end - start}')
 
start = time.time()
testing = get_labeled_data(MNIST_data_path, bTrain=False)
end = time.time()
...
for name in population_names:
    ...
    if test_mode or weight_path[-8:] == 'weights/':
        theta_vals = np.load(weight_path + 'theta_' + name + ending + '.npy', allow_pickle=True)
        neuron_groups[name+'e'].theta = theta_vals * b.volt
